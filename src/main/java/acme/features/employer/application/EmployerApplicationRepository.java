...
public interface EmployerApplicationRepository extends AbstractRepository {
	...
	@Query("select a from Application a where a.job.employer.id = ?1 order by a.referenceNumber desc")
	Collection<Application> findManyByReferenceNumber(int employerId);
